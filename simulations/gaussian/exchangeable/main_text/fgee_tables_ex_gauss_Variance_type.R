...
library(tidyverse)
library(ggpubr)
process.wd <- "/Users/Desktop/NIMH Research/Code/"
...
cv.vec <- c("fastkfold")
var.vec <- c(TRUE, FALSE, "fastBoot") # , "fastBoot"
...
x=dat %>% tibble %>%  
  ...
  dplyr::filter(n_sim %in% n_included) %>%
  ...
  dplyr::mutate(key=fct_relevel(key, c("One-Step","GLS-Ex","GLS-Ind", "Marginal"))) %>%
  as.data.frame()
...
x$key <- droplevels(x$key, c("robust", "comb", NA, "ar1", "NA", 
                             "GLS-Ex","GLS-Ind", "Marginal")) # *** can just add these back if needed
x$cv <- droplevels(x$cv, c("kfold")) # "TRUE", 
...
x %>% dplyr::as_tibble() %>%
  group_by(N_i, n_sim, sigma_sq, key, var.type,cv) %>%
...
x$key <- droplevels(x$key, c("robust", "comb", NA, "ar1", "NA", 
                             "GLS-Ex","GLS-Ind", "Marginal", "pffr")) # *** can just add these back if needed
x$cv <- droplevels(x$cv, c("kfold")) # "TRUE", 

x$var.type <- droplevels(x$var.type, c("FALSE")) # "TRUE", 
...
x$key <- droplevels(x$key, c("robust", "comb", NA, "ar1", "NA", 
                             "GLS-Ex","GLS-Ind", "Marginal")) # *** can just add these back if needed
x$var.type <- droplevels(x$var.type, c("TRUE", "NA", NA)) # "TRUE",
x$var.type <- relevel(x$var.type, ref = "fastBoot")
...
x %>% dplyr::as_tibble() %>%
  group_by(N_i, n_sim, sigma_sq, key, r,var.type) %>%
  summarize(mean(value, na.rm=TRUE), nn = n()) %>% print(n = "Inf")
