...
save.folder <- "/home/fgee/fgee_sim_gauss_ex" # photometry sims # 26 had fixed knots, 27 had varied
...
dat_sim$time <- rep(1, nrow(dat_sim$data$Y)) # place-holder
...
if(comparison_methods){
  ...
  # dd <- data.frame(X1 = 1, cue = 1, yindex.vec = 1:L)
  dd <- data.frame(X1 = 1, X2 = 1, yindex.vec = 1:L)
  fit$model$Ve <- fit$model$Vc <- fit$model$Vp <- fit$Vp <- fit$vb
  # preds <- predict(fit$model, dd, type="terms", se.fit=TRUE)
...
if(cov.type == "exchangeable"){
  ...
  exch.estim <- fgee(formula = Y ~ X1 + X2, 
                     ...
                     numLongiPoints=L,
                     control=list(FPCA1.pve=0.95, 
                                  FPCA1.knots=nknots_min),
                     corstr = "exchangeable")
  ...
  fit$qn <- joint.qn(glmfit = fit, NN.sim = 10000)
  ...
  cnt <- which(colnames(res_mat) == "Li_pCI_int") - 1
  # average of CI inclusion over reward period
  res_mat[iter, cnt+1] <- mean( naive_incl[1, ] )  # whether pointwise CI contains estimate
  res_mat[iter, cnt+2] <- mean( joint_incl[1, ] )  # whether joint CI contains estimate
