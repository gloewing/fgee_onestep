...
#######################################################
wd <- "/Users/Desktop/Research/fgee_sim_gauss_ar"
setwd(wd)
library(dplyr)
library(latex2exp)
library(kableExtra)
...
process.ind <- TRUE # process files flag
#-----------------------
# simulation parameters
reps <- 300 # replicates of simulations
boots <- 5000 # for bootstrap permutation test
nknots <- 10 
cv.vec <- c("fastkfold", "kfold")
var.vec <- c("fastBoot", TRUE, FALSE) # FALSE, TRUE, "fastBoot"
...
method_vec <- c("fGEE_", "pffr_", "robust_", "exact_", "unwght_", "ar1_", "comb_")
ref.nm <- "pffr_"
...
ls_j <- ls_rmse_rel <- ls_rmse_slp <- ls_pci_slp <- list()
ls_time <- ls_rmse <- ls_pci <- ls_jci <- ls_p <- list()
#-----------------------
            ...
            for(N_i in NiVec){
              for(rho.smooth in rhoVec){
                ...
                if(file.exists(flNm)){
                  ...
                  # power joint
                  nm_vec <- paste0(method_vec, "jSig")
                  ls_j[[cnt]] <- cbind(gather(d[,nm_vec]), 
                                       cv, var.type, n_sim, N_i, rho.smooth, sigma_sq, l, r )
...
x=dat %>% tibble %>%  
  ...
  dplyr::mutate(key=str_replace_all(key, fixed("fGEE"), "One-Step"),
                key=str_replace_all(key, fixed("exact"), "GLS-AR1"),
                key=str_replace_all(key, fixed("unwght"), "GLS-Ind")
                ) %>%
  dplyr::mutate(key=fct_relevel(key, c("One-Step","GLS-AR1","GLS-Ind"))) %>%
  as.data.frame()
...
x$key <- droplevels(x$key, c(NA, "ar1", "NA", "comb", "robust"))
...
###########################
# Power joint CI Table
###########################
# factors 
dat <- do.call(rbind, ls_j)
dat$cv <- as.factor(dat$cv)
dat$var.type <- as.factor(dat$var.type)
dat$n_sim <- as.factor(dat$n_sim)
dat$N_i <- as.factor(dat$N_i)
...
x$cv <- as.factor(x$cv)
x$r <- as.factor(x$r)
x$sigma_sq <- as.factor(x$sigma_sq)
