#----------------------------
# SL data analysis with FGEE - whisker
#----------------------------
library(data.table)
library(ggplot2)
library(gridExtra)
...
cov.type <- "ar1"
fit.ar <- fun.gee1step(formula = Y ~ whisk,
                    data = d,
                    cluster = "ID",
                    family="binomial",
                    algorithm = "bam",
...
plot.ar1 <- fgee.plot(fit.ar, 
                     xlab = "Time (sec)", 
                     title_names = c("AR1: Intercept", "AR1: Whisker"), 
                     align_x = 1, 
                     x_rescale = hz, 
                     y_val_lim = 1,
                     return = FALSE) 
...
timeStart <- Sys.time()
fit.ex <- fun.gee1step(formula = Y ~ whisk,
                    data = d,
                    cluster = "ID",
                    family="binomial",
                    algorithm = "bam",
...
plot.ex <- fgee.plot(fit.ex, 
                     xlab = "Time (sec)", 
                     title_names = c("Exch: Intercept", "Exch: Whisker"), 
                     align_x = 1, 
                     x_rescale = hz, 
                     y_val_lim = 1,
                     return = FALSE) 
...
plot.sand <- fgee.plot(fit, 
                       xlab = "Time (sec)", 
                       title_names = c("Ind: Intercept", "Ind: Whisker"), 
                       align_x = 1, 
                       x_rescale = hz, 
                       y_val_lim = 1,
                       return = FALSE) 
...
setwd(save.wd)
ggsave( "whisk-comb.pdf",
        plot = plt_comb,
        width = 5,
        height = 7)

saveRDS(plot.list, file = "model.list_Whisker.Rdata")

# to plot saved objects use the following
mod.list <- readRDS(file="model.list_Whisker.Rdata")
